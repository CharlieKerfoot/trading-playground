...
def _launch_web(port: int = 8000) -> int:
  """Start the FastAPI server and SvelteKit dev server, then open the browser."""
  import subprocess
  import sys
  import time
  import webbrowser
...
def sync(limit: int):
  ...
  result = cache.sync(limit=limit, on_progress=on_progress)
  console.print()
...
@cli.command()
@click.option("--agent", default="rule", help="Agent type: rule, claude, rl, random, or rl:path/to/model")
@click.option("--episodes", default=50, help="Number of episodes to run")
def run(agent: str, episodes: int):
  """Run an agent on cached market data and display results."""
  ...
  from polymarket_playground.eval.metrics import (
    avg_pnl, max_drawdown, sharpe_ratio, win_rate,
  )

  cache = MarketCache()
  stats = cache.stats()
  if stats["total_markets"] == 0:
    console.print("[red]No markets in cache.[/red] Run `cli.py sync` first.")
    return
  ...
  console.print(
    f"Running [bold]{agent}[/bold] agent for {episodes} episodes "
  ...
  table.add_row("Win Rate", f"{win_rate(results):.1%}")
  table.add_row("Avg P&L", f"{avg_pnl(results):+.6f}")
  table.add_row("Max Drawdown", f"{max_drawdown(results):+.6f}")
  console.print(table)
...
def train(timesteps: int, algorithm: str, save_path: str | None):
  """Train an RL agent using SB3."""
  from pathlib import Path
  from polymarket_playground.core.env import TradingEnv
  from polymarket_playground.data.market_cache import MarketCache
