...
class LiveExecutor(BaseExecutor):
    ...
    def get_balance(self) -> float:
        """Get USDC balance by checking open positions and available funds.

        Uses the CLOB API's balance endpoint. Falls back to 0.0 on error.
        """
        try:
            import requests
            ...
            logger.warning("Balance check returned status %d", resp.status_code)
            return 0.0
